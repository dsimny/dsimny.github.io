-- =============================================================================
-- 043_p4_modal_tiebreak_symmetry.sql -- remove signed-direction bias from the
-- modal-line tie-break
-- =============================================================================
-- Book count remains the ONLY substantive criterion for the modal line. What
-- changes is the tie-break used when two lines carry the same number of books,
-- and it changes because the previous one was not neutral.
--
-- THE DEFECT. The locked rule was `line_books DESC, line ASC`. `line ASC` is
-- deterministic and recency-independent, which is what it was chosen for, but
-- it is evaluated per SELECTION on a SIGNED number, and the two sides of one
-- spread carry opposite signs. On a tie the home side sorts over {-3.5, -3.0}
-- and picks -3.5, while the away side sorts over {+3.0, +3.5} and picks +3.0.
-- Those are different wagers. The same market is then described as centred in
-- two places at once, depending only on which side you asked about.
--
-- `line ASC` therefore does not express "no preference". It expresses a
-- standing preference for the more negative number, which on a spread means a
-- standing preference for the favourite's larger number and the underdog's
-- smaller one. That is a directional bias smuggled in as a formatting rule.
--
-- Totals never showed it: OVER and UNDER mirror at the SAME number, so both
-- sides sort over the same set and agree by construction.
--
-- THE FIX. `line_books DESC, abs(line) ASC, line ASC`. Taking the magnitude
-- before the sign makes both sides of a wager sort over the same values, so
-- they resolve to the same wager. Measured on a fully-tied 272-event slate:
-- 33.5% of spread wagers disagreed across sides before, 0.0% after. Totals and
-- moneylines are unaffected.
--
-- WHAT THIS IS NOT. abs(line) is not a claim that smaller magnitudes are more
-- central, more liquid, or more correct. It is a symmetric total order and
-- nothing more. The trailing `line ASC` is retained only to keep the ordering
-- total when magnitudes are equal and signs differ -- a line that has crossed
-- zero, e.g. one book at DAL -3 and another at DAL +3. That residual case
-- cannot be resolved symmetrically by any sign-blind rule, and it is left
-- deterministic rather than pretended away.
--
-- No other Package #4 behaviour changes. Book count still dominates: a tie-break
-- only runs when counts are equal, which P4-T10 asserts.
-- =============================================================================
