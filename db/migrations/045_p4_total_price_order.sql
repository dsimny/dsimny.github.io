-- =============================================================================
-- 045_p4_total_price_order.sql -- make the best-price ordering TOTAL
-- =============================================================================
-- Completes the ordering rule 044 introduced. The rule, stated in full:
--
--     1. positive beats negative
--     2. among positive, larger is better        (+101 beats +100)
--     3. among negative, closer to zero is better (-101 beats -102)
--
-- `olp_price_payout` already implements 2 and 3 exactly, and implements 1 for
-- every pair EXCEPT one. Positive prices pay `p/100 >= 1.0`; negative prices pay
-- `100/|p| <= 1.0`. So positive always outranks negative -- unless both sit on
-- the boundary:
--
--     +100  ->  100/100 = 1.0
--     -100  ->  100/100 = 1.0
...
-- At that single pair the payout is genuinely identical -- both are even money,
-- so neither choice costs anything -- and the ordering fell through to
-- `sportsbook ASC`, which is arbitrary rather than wrong.
--
-- Rule 1 is nonetheless stated as absolute, so it is now enforced as absolute:
-- `(price > 0) DESC` sits between the payout key and the alphabetical
-- tie-break. It can only ever fire on +100 vs -100. It is recorded here so a
-- later reader does not mistake it for an economic claim -- it is not one, it
-- makes an arbitrary choice deterministic and canonical.
--
-- SCOPE. The same two ORDER BY clauses 044 touched, and only their ordering:
--   * canonical_market.best      -- canonical best_price
--   * executable_market.exec_best -- executable best_price, which is ALSO the
--                                   promoted/fallback book after a quote is
--                                   superseded by a line move
-- `best_ties` (best_price_book_count) compares integer prices with `=` and was
-- never affected by any of this.
--
-- Audit of every olp_american_profit() call site, at this migration:
--   014, 030  place_ticket_rpc / reschedule  -- MONEY (frozen potential_profit). Correct.
--   028       ticket_closing_line_value      -- RANKING, Package #2, FROZEN.
--                                              Same rounding collision. Reported,
--                                              not fixed. Needs its own decision.
--   038-043   superseded by 044/045.
-- No other ORDER BY, MAX, MIN, GREATEST, LEAST, rank() or row_number() in the
-- schema ranks on price.
-- =============================================================================
