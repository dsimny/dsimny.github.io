-- =============================================================================
-- 048_p4_modal_without_a_join.sql -- keep 047's semantics, restore the
-- planner's cardinality estimate
-- =============================================================================
-- PURE PERFORMANCE. Byte-exact with 047 on the captured live board: 2,480 rows
-- each, symmetric difference of the full-row JSON multisets = 0.
--
-- WHAT 047 COST, AND WHERE. Bisected against 046 one logical change at a time,
-- same session, same dataset, same settings:
--
--     V0  046 baseline                                   canonical 1.12 s
--     V1  + ref_modal CTE present but unused                        0.91 s
--     V2  + modal_line taken from ref_modal and mirrored            2.62 s  <--
--     V3  + modal_line_book_count via the LEFT JOIN                 2.56 s
--     V4  full 047                                                  2.62 s
--
-- Neither adding a CTE nor the `IS NOT DISTINCT FROM` count lookup cost
-- anything. The mirroring JOIN did, and EXPLAIN says exactly why:
--
--     CTE Scan on modal    046:  est  200 / actual 1634   x1 loop
--                          047:  est    1 / actual 1634   x2480 loops
--
-- 046 built `modal` with DISTINCT ON over one CTE, which PostgreSQL estimates
-- at its default 200 groups. 047 built it by joining two statistics-free CTEs
-- (`per_sel_count` x `events` x `ref_modal`); with no statistics on either side
-- the join selectivity bottoms out and the estimate collapses to ONE row. The
-- planner then judged a nested loop free and rescanned the 1,634-row CTE once
-- per output row -- 2,480 times.
--
-- This is the third time in Package #4 that a statistics-free CTE has produced
-- a nonsense estimate and a quadratic plan (042, 046, now 048). The lesson is
-- narrow and worth stating plainly: inside this pipeline, prefer a formulation
-- whose row count the planner can already estimate -- an aggregate, a window, a
-- DISTINCT ON over ONE relation -- over a join between CTEs.
--
-- THE FIX. Express the mirroring without a join. first_value() carries the
-- reference side's answer into every row of its (event, market_type) partition,
-- so `modal` stays exactly what 046 made it -- a DISTINCT ON over line_pop, one
-- row per (event, market_type, selection) -- and the estimate stays at 200.
--
--     CTE Scan on modal    048:  est  200 / actual 1634   x1 loop
--
-- Ordering the partition with the reference rows first and then by the modal
-- rule itself means the partition's first row IS the reference modal. The CASE
-- inside first_value() yields NULL when a market has no reference-side quote at
-- all, so such a market fails closed instead of silently adopting the other
-- side's number.
--
-- SEMANTICS UNCHANGED FROM 047. Bookmaker count is still the only substantive
-- criterion; `abs(line) ASC, line ASC` are still the tie-breakers; the modal
-- line is still decided once per wager from the home side (OVER for totals) and
-- mirrored; modal_line_book_count is still the books quoting THAT side at the
-- centre, NULL where the side does not offer it.
--
-- Measured, same session, same board:
--     canonical_market   047 1.96 s  ->  048 0.72 s
-- executable_market and market_intelligence are unchanged in form; they get
-- faster only because canonical does.
-- =============================================================================
...
newest AS MATERIALIZED (
    SELECT DISTINCT ON (s.event_id, s.market_type, s.selection, s.line, s.sportsbook)
           ...
           CASE s.market_type
               WHEN 'SPREAD'    THEN s.selection = e.home_team
               WHEN 'TOTAL'     THEN s.selection = 'OVER'
...
line_pop AS (
    SELECT event_id, market_type, selection, line,
           ...
           bool_or(is_ref_side)       AS is_ref_side
    FROM newest
    GROUP BY 1, 2, 3, 4
),
-- The modal line is a property of the WAGER: decided ONCE per
-- (event, market_type) from a fixed reference side -- home team for SPREAD,
-- OVER for TOTAL -- and mirrored to the other side, so the two can never name
-- different wagers. See 047.
--
-- Expressed as a window over line_pop rather than a join to a reference CTE.
-- Both give identical results; the join form collapsed the planner's estimate
-- for this node from 200 rows to 1, which made it nested-loop and rescan the
-- whole CTE once per output row. See 048.
--
-- first_value() carries the reference side's answer into every row of the
-- partition. Ordering the partition with the reference rows first and then by
-- the modal rule means the first row IS the reference modal. The CASE inside
-- first_value yields NULL when a market has no reference-side quote at all,
-- which fails closed rather than silently adopting the other side's number.
modal AS MATERIALIZED (
    SELECT DISTINCT ON (event_id, market_type, selection)
           event_id, market_type, selection,
           target AS modal_line,
           -- Books quoting THIS selection at the wager's centre; NULL when this
           -- side does not offer that number.
           CASE WHEN line IS NOT DISTINCT FROM target THEN line_books END
               AS modal_line_book_count
    FROM (
        SELECT w.*,
               CASE w.market_type
                   WHEN 'MONEYLINE' THEN NULL
                   WHEN 'TOTAL'     THEN w.ref_line
                   WHEN 'SPREAD'    THEN CASE WHEN w.is_ref_side THEN  w.ref_line
                                                                 ELSE -w.ref_line END
               END::numeric(7,2) AS target
        FROM (
            SELECT lp.*,
                   first_value(CASE WHEN lp.is_ref_side THEN lp.line END) OVER (
                       PARTITION BY lp.event_id, lp.market_type
                       -- reference side first, then the modal rule itself:
                       -- book count is the ONLY substantive criterion; abs(line)
                       -- and line are deterministic tie-breakers -- see 043.
                       ORDER BY (NOT lp.is_ref_side),
    ...
    ORDER BY event_id, market_type, selection,
             (line IS NOT DISTINCT FROM target) DESC
),
...
-- Same wager-level rule as canonical_market, expressed the same way, so
-- line_movement compares like with like and neither view collapses the
-- planner's estimate for this node. See 047 for the rule and 048 for the form.
opening_modal AS MATERIALIZED (
    SELECT DISTINCT ON (event_id, market_type, selection)
           event_id, market_type, selection,
           CASE market_type
               WHEN 'MONEYLINE' THEN NULL
               WHEN 'TOTAL'     THEN ref_line
               WHEN 'SPREAD'    THEN CASE WHEN is_ref_side THEN  ref_line
                                                           ELSE -ref_line END
