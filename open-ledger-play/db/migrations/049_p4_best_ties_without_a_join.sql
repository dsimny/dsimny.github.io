-- =============================================================================
-- 049_p4_best_ties_without_a_join.sql -- the same lesson, applied to the one
-- place it was still true
-- =============================================================================
-- PURE PERFORMANCE. Byte-exact with 048; no column changes value.
--
-- FOUND BY P4-T29, on its first run after the guard was generalised from the
-- `modal` node to every node in the plan. This defect is NOT from 047 or 048 --
-- it has been in the pipeline since 038, latent, and 046 did not remove it:
-- MATERIALIZED stops a CTE being RECOMPUTED, but a nested loop can still
-- RESCAN the materialised tuplestore.
--
--     dense fixture, canonical_market:
--         CTE Scan on best_ties    est 1 / actual 600 / loops 600
--         ratio 600x underestimate, 360,000 rows of repeated work
--
-- `best_ties` counted the books sitting on the best price by joining `newest`
-- to `best`. Both are CTEs, neither carries statistics, so the join estimate
-- bottoms out at one row and the planner judges a nested loop free -- exactly
-- the failure 047 hit at the `modal` node and 048 fixed there.
--
-- THE FIX, identical in spirit to 048: express it over ONE relation. A window
-- partitioned by the canonical key names the winning price in the same pass,
-- and a FILTER counts the books matching it. The window ORDER BY is
-- character-for-character the ordering `best` uses, so first_value() cannot
-- disagree with the price `best` selects.
--
-- WHY IT WAS NOT URGENT, AND WHY IT IS STILL WORTH FIXING. At present sizes the
-- rescan is cheap and canonical_market runs in well under a second, so nothing
-- was visibly wrong. That is precisely what "latent" means, and it is what the
-- structural guard exists to surface: 047's identical collapse was equally
-- invisible on a synthetic board right up until the dense live board turned it
-- into twenty minutes.
--
-- SEMANTICS UNCHANGED. best_price_book_count is still the number of distinct
-- books quoting the best price at that (event, market_type, selection, line).
-- =============================================================================
...
-- How many books sit on the best price. Computed with a window over `newest`
-- rather than by joining `newest` to `best`: two statistics-free CTEs joined
-- together estimate at 1 row, which makes a nested loop look free. See 049.
--
-- The window ORDER BY is character-for-character the ordering `best` uses, so
-- first_value() names the same winning price `best` selects.
best_ties AS MATERIALIZED (
    SELECT event_id, market_type, selection, line,
           count(*) FILTER (WHERE price = winning_price) AS best_price_book_count
    FROM (
        SELECT n.event_id, n.market_type, n.selection, n.line, n.price,
               first_value(n.price) OVER (
                   PARTITION BY n.event_id, n.market_type, n.selection, n.line
                   ORDER BY public.olp_price_payout(n.price) DESC,
                            (n.price > 0) DESC,
                            n.sportsbook ASC
               ) AS winning_price
        FROM newest n
