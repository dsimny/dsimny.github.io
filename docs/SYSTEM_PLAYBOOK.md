...
| Road wOBA Suppression (12% run tax) | **LIVE END-TO-END (detection v0.9; tax v0.12 at 8%, 2026-07-29)** | fetch_data.py pulls every team's trailing-14-day wOBA plus the league baseline (MLB API byDateRange components; no Statcast needed); the trigger (gap > .035) and the tax are computed and printed on every card. The tax is **8%, not the playbook's 12%** — sized by the full-season sweep (2026-04-01..07-27, 1454 games, 152 triggered): the untaxed model over-projected triggered games' totals by +0.31 runs, 8% erased the bias (−0.01) and won Brier on all games (0.2481→0.2477) and the fired subset (0.2466→0.2439), while 12% over-corrected (−0.18 bias, worse Brier). Re-tune only via the "Backtest sweep" Actions workflow on a full season. |
...
| Thermal / Venue Launch Penalty | **LIVE on the totals paper track (engine v0.10, 2026-07-29)** | Game-hour forecasts per outdoor park (Open-Meteo; roofed parks excluded) drive a temperature multiplier (±0.4%/°F around 70°F, clamped) plus the playbook's hot-day trigger: ≥85°F versus a fly-ball starter (air-out-share proxy for FB%>42%) adds a 5% kicker to runs scored off that starter. The literal 35% HR/FB tax cannot map onto a run-rate model, so this is the documented adaptation. Applied ONLY to the paper totals pick via a second, separately-seeded sim — the staked board is bit-identical with or without weather. The totals ledger stamps wx_applied so the win%/CLV split validates (or kills) the adjustment before anything staked ever sees weather. |
