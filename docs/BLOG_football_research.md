...
**Publishing note.** `blog.py` currently has two paths — a slate post on game
days and the evergreen rotation on off days — and no "publish this specific
piece today" hook. Appending this to `blog_evergreen.POSTS` would work but would
surface it on an arbitrary off day, which is wrong for an announcement. Either
add a one-off path or publish it by hand. That decision is Daniel's; the copy is
ready either way.

**Every number below is checked against the repo.** Sources are
`docs/FOOTBALL_RESULT_T24.md`, `docs/FOOTBALL_RESULT_PRICE.md`, and
`data/football/price_test.json`.
