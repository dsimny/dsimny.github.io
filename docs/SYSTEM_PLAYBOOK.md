...
| Road wOBA Suppression (12% run tax) | **DETECTION LIVE (engine v0.9, 2026-07-29); tax OFF, backtest-gated** | fetch_data.py pulls every team's trailing-14-day wOBA plus the league baseline (MLB API byDateRange components; no Statcast needed), and the trigger (gap > .035) is computed and printed on every card. The run tax itself is WOBA_TAX=0: a hardwired 12% off a 14-day sample cuts against the engine's validated regression-to-mean philosophy, so the tax size is tuned via `backtest.py --sweep woba_tax:0,0.04,0.08,0.12` over a full season (watching totals calibration too) before any non-zero value ships. Cards flagged today say "detection only" explicitly. |
